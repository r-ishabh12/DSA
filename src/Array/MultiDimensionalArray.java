...
public class MultiDimensionalArray {
    ...
    //Pefix sum over columns and rows both
    static void findPrefixSumMatirxRowColumn(int[][] matrix){
        int r = matrix.length;
        int c = matrix.length;

        //traverse horizontallly to calculate row-wise prefix sum
        for (int i = 0; i < r; i++ ){
            for (int j = 1; j < c; j++){
                matrix[i][j] += matrix[i][j-1];
            }
    ...
    static int findSum3(int[][] matrix,int l1, int r1, int l2, int r2){
        ...
        findPrefixSumMatirxRowColumn(matrix);
    ...
    public static void main(String[] args) {
        //How to initialize 2D Array.Array
